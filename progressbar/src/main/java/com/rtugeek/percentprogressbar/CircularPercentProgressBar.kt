...
class CircularPercentProgressBar : View {
    companion object {
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(
        ...
        if (attrs != null) {
            val typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.CircularPercentProgressBar)

            totalValue = typedArray.getInt(R.styleable.CircularPercentProgressBar_totalValue, 100)
            totalColor = typedArray.getColor(R.styleable.CircularPercentProgressBar_totalColor, 0)
            totalWidth = typedArray.getDimensionPixelSize(
                R.styleable.CircularPercentProgressBar_totalWidth,
                16
            ).toFloat()

            progressValue =
                typedArray.getInt(R.styleable.CircularPercentProgressBar_progressValue, 0)
            progressColor =
                typedArray.getColor(R.styleable.CircularPercentProgressBar_circularProgressColor, 0)
            progressWidth = typedArray.getDimensionPixelSize(
                R.styleable.CircularPercentProgressBar_progressWidth,
                16
            ).toFloat()
            progressRoundCap = typedArray.getBoolean(
                R.styleable.CircularPercentProgressBar_progressRoundCap,
                false
            )

            progressTextEnabled = typedArray.getBoolean(
                R.styleable.CircularPercentProgressBar_progressTextEnabled,
                false
            )
            progressTextType = typedArray.getInt(
                R.styleable.CircularPercentProgressBar_progressTextType,
                PROGRESS_TEXT_TYPE_PROGRESS
            )
            progressTextSize = typedArray.getDimensionPixelSize(
                R.styleable.CircularPercentProgressBar_progressTextSize,
                0
            ).toFloat()
            progressTextColor =
                typedArray.getColor(R.styleable.CircularPercentProgressBar_progressTextColor, 0)
            clockwise =
                typedArray.getBoolean(R.styleable.CircularPercentProgressBar_clockwise, true)

            // Set default to linear interpolator
            val interpolatorResId = typedArray.getResourceId(
                R.styleable.CircularPercentProgressBar_progressInterpolator,
            ...
            fillColor = typedArray.getColor(R.styleable.CircularPercentProgressBar_fillColor, 0)

            startAngle =
                typedArray.getFloat(R.styleable.CircularPercentProgressBar_startAngle, 270f)
            animate = typedArray.getBoolean(R.styleable.CircularPercentProgressBar_animate, false)
            animateDuration =
                typedArray.getInt(R.styleable.CircularPercentProgressBar_animateDuration, 300)
                    .toLong()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (progressTextEnabled) {
            ...
            val yPosition =
                centerY - ((paintProgressText.descent() + paintProgressText.ascent()) / 2)
        ...
        if (totalValue != 0 && progressValue != 0 && progressValue <= totalValue) {
            val progressSweepAngle =
                if (totalValue == progressValue) 360f else ((360f / totalValue) * progressValue)
            if (clockwise) {
    ...
    override fun onRestoreInstanceState(state: Parcelable?) {
        if (state is Bundle) {
            ...
            progressInterpolator =
                AnimationUtils.loadInterpolator(context, progressInterpolatorResId)
            setupPaint()
